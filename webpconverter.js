...
//enwebp() return which platform webp library should be used for conversion
 exec(enwebp(),query.split(/\s+/), {shell: true}, function (error, stdout, stderr) //execute command 
 	{ 
 		if(error)
 		{
 			callback(error);//return error code
 		}
 		else 
 		{
  			callback(null);//return success code
	    }
 });
};
...
//dewebp() return which platform webp library should be used for conversion
 exec(dewebp(),query.split(/\s+/), {shell: true}, function (error, stdout, stderr) //execute command 
 	{ 
 		if(error)
 		{
 			callback(error);//return error code
